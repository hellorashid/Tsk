...
interface TimelineViewProps {
  ...
  onAddEvent?: (eventData: ScheduleCardInput) => Promise<ScheduleCardData>;
...
const TimelineView: React.FC<TimelineViewProps> = ({
  ...
  const currentTimeMinutesRef = useRef(currentTimeMinutes);
  ...
  useEffect(() => {
    currentTimeMinutesRef.current = currentTimeMinutes;
  }, [currentTimeMinutes]);
  ...
  const positionedEvents = useMemo(() => {
    ...
    const eventData: PositionedTimelineEvent[] = eventsToPosition.map((event) => {
    ...
    const checkOverlap = (a: PositionedTimelineEvent, b: PositionedTimelineEvent) => {
      return a.startMinutes < b.endMinutes && b.startMinutes < a.endMinutes;
    };

    // Group overlapping events
    const groups: PositionedTimelineEvent[][] = [];
    ...
    groups.forEach(group => {
      ...
      const columns: PositionedTimelineEvent[][] = [[]];
    ...
    eventData.forEach(event => {
      ...
      event.hasAdjacentAbove = hasAdjacentTopLeft || hasAdjacentTopRight;
      event.hasAdjacentBelow = hasAdjacentBottomLeft || hasAdjacentBottomRight;
      event.adjacentCorners = {
    ...
    const handleMouseUp = async () => {
      ...
      if (capturedStartMinutes !== null && capturedEndMinutes !== null) {
        ...
        if (duration >= 15) {
          ...
          const eventData: ScheduleCardInput = {
            title: 'New Event',
            start: {
              dateTime: startDate.toISOString(),
  ...
  useEffect(() => {
    ...
    if (savedScroll !== null) {
      ...
      const scrollPosition = (currentTimeMinutesRef.current - timeRange.startMinutes) * pixelsPerMinute - 200;
  ...
  }, [selectedDate, isToday, pixelsPerMinute, timeRange.startMinutes]);
            ...
            {positionedEvents.length === 0 ? (
              ...
              <div className="relative" data-timeline-content style={{ minHeight: `${timelineHeight}px` }}>
                {positionedEvents.map((posEvent) => {
                  const { event, startMinutes, duration, column, totalColumns, mergedCompletionEvents, hasAdjacentAbove, hasAdjacentBelow, adjacentCorners } = posEvent;
                  return (
                    <TimelineEventCard
                      key={event.id}
                      event={event}
